...
No need to worry, you won't. All of LobeChat's chat records are stored in your local browser. Therefore, when redeploying AiPMChat using Docker, your chat records will not be lost.
...
output=$(docker pull aipmhub/aipm-chat:latest 2>&1)
...
echo "$output" | grep -q "Image is up to date for aipmhub/aipm-chat:latest"
...
echo "Started: $(docker run -d --network=host --env-file /path/to/lobe.env --name=lobe-chat --restart=always aipmhub/aipm-chat)"

# Print the update time and version
echo "Update time: $(date)"
echo "Version: $(docker inspect aipmhub/aipm-chat:latest | grep 'org.opencontainers.image.version' | awk -F'"' '{print $4}')"

# Clean up unused images
docker images | grep 'aipmhub/aipm-chat' | grep -v 'latest' | awk '{print $3}' | xargs -r docker rmi > /dev/null 2>&1
